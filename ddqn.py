...
class DQNAgent:
    ...
    def __init__(self, state_space, action_space, max_memory_size, batch_size, gamma, lr, dropout, exploration_max, exploration_min, exploration_decay, double_dq, pretrained):
        ...
        if self.double_dq:
            ...
            self.copy = 5000 # updates weights every 5000 steps
            self.step = 0
        else:
    ...
    def remember(self, state, action, reward, state2, done):
        '''pushes all updates from previous state to our memory files'''
    ...
    def experience_replay(self):
        ...
        if self.double_dq and self.step % self.copy == 0:
            self.copy_model() # remove function
        ...
        if self.target_net(STATE2) is None:
            print("WHYYYY")

        self.optimizer.zero_grad()
        if self.double_dq:
            # Double Q-Learing target is Q*(S, A) <- r + y max_a Q_target(S', a)
        ...
        # makes sure the exploration rate is always at least 'exploration min'
...
def train(training_mode=True, pretrained=False):
    ...
    for ep_num in tqdm(range(num_episodes), bar_format='{l_bar}{bar:20}{r_bar}{bar:-10b}'):
        state = env.reset()
        ...
        print(f'Total reward after episode {ep_num + 1} is {total_rewards[-1]}')
        num_episodes += 1
        ...
        if num_episodes > 500:
            ...
            plt.plot([0 for _ in range(50)] + np.convolve(total_rewards, np.ones((500,))/500
                                                          ,mode = "valid").tolist())
            plt.show()
