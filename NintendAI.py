...
def run(training_mode, pretrained):
    ...
    env.reset()
    total_rewards = []
    open(f'training_log.txt', 'w').write(f'ep_num\tsteps\taction\treward\tterminal\ttotal_reward\n')
    ...
    for ep_num in tqdm(range(num_episodes)):
        ...
        while True:
            ...
            state = state_next
            open(f'training_log.txt', 'a').write(f'{ep_num}\t{steps}\t{action.item()}\t{reward.item()}\t{terminal}\t{total_reward}\n')
            if terminal:
                break
